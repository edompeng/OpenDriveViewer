...
bool RayIntersectsAABB(const QVector3D& origin, const QVector3D& dir, 
                       const QVector3D& min_b, const QVector3D& max_b, float& t_near) {
    float tmin = -std::numeric_limits<float>::infinity();
    float tmax = std::numeric_limits<float>::infinity();

    // X axis
    float invDx = 1.0f / (std::abs(dir.x()) < 1e-9f ? (dir.x() < 0 ? -1e-9f : 1e-9f) : dir.x());
    ...
    // Y axis
    float invDy = 1.0f / (std::abs(dir.y()) < 1e-9f ? (dir.y() < 0 ? -1e-9f : 1e-9f) : dir.y());
    ...
    // Z axis
    float invDz = 1.0f / (std::abs(dir.z()) < 1e-9f ? (dir.z() < 0 ? -1e-9f : 1e-9f) : dir.z());
...
SpatialIndexData BuildSpatialIndex(
    ...
    for (const auto& view : layer_views) {
        ...
        for (size_t i = 0; i + 2 < indices.size(); i += 3) {
            ...
            TriTemp t;
            uint32_t layer_tag = view.resolve_layer_tag ? view.resolve_layer_tag(i0) : view.default_layer_tag;
            t.encoded = (layer_tag << 28) | (static_cast<uint32_t>(i/3) & 0x0FFFFFFF);
    ...
    struct BuildTask { int start, end; uint32_t node_idx; };
    ...
    while (!stack.empty()) {
        ...
        if (count <= 16) { // Leaf threshold
            ...
            std::nth_element(all_tris.begin() + task.start, all_tris.begin() + mid, all_tris.begin() + task.end,
...
std::optional<SpatialPickResult> PickFromSpatialIndex(
    ...
    struct StackNode { uint32_t node_idx; float t_near; };
    std::stack<StackNode> stack;

    float t_root = 0;
    if (RayIntersectsAABB(ray_origin, ray_dir, index_data.nodes[0].min_bound, index_data.nodes[0].max_bound, t_root)) {
        stack.push({0, t_root});
    }
    ...
    while (!stack.empty()) {
        ...
        if (node.tri_count > 0) {
            ...
            for (uint32_t i = 0; i < node.tri_count; ++i) {
                ...
                QVector3D v0(mesh->vertices[v0_idx][0], mesh->vertices[v0_idx][1], mesh->vertices[v0_idx][2]);
                QVector3D v1(mesh->vertices[mesh->indices[base+1]][0], mesh->vertices[mesh->indices[base+1]][1], mesh->vertices[mesh->indices[base+1]][2]);
                QVector3D v2(mesh->vertices[mesh->indices[base+2]][0], mesh->vertices[mesh->indices[base+2]][1], mesh->vertices[mesh->indices[base+2]][2]);
            ...
            bool hit_left = RayIntersectsAABB(ray_origin, ray_dir, index_data.nodes[node.left_child].min_bound, index_data.nodes[node.left_child].max_bound, t_left);
            bool hit_right = RayIntersectsAABB(ray_origin, ray_dir, index_data.nodes[node.right_child].min_bound, index_data.nodes[node.right_child].max_bound, t_right);
...
std::vector<RaycastHitPoint> RaycastAllHits(
    ...
    while (!stack.empty()) {
        ...
        float t_near = 0;
        if (!RayIntersectsAABB(ray_origin, ray_dir, index_data.nodes[node_idx].min_bound, index_data.nodes[node_idx].max_bound, t_near)) continue;
